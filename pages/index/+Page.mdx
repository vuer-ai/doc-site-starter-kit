...
# Documentation<br /><span className="text-indigo-600">Starter Kit</span>
...
Ship beautiful docs fast — without wrestling with build tools.
  ...
  <div className="flex flex-wrap gap-3">
    ...
    <a
      href="/installation"
      className="inline-flex items-center gap-2 px-5 py-2.5 bg-white text-gray-700 text-sm font-medium rounded-xl border border-gray-200 hover:bg-gray-50 transition-colors"
    >
      Installation guide
    </a>
  </div>
...
{/* Quick install */}
<div className="mb-16 p-5 rounded-xl" style={{ backgroundColor: 'rgb(var(--color-bg-secondary))', border: '1px solid rgb(var(--color-border))' }}>
  <p className="text-xs font-medium uppercase tracking-wider mb-3" style={{ color: 'rgb(var(--color-text-muted))' }}>Quick start</p>
  <CodeBlock language="bash" code={`pnpm create vike@latest my-docs\ncd my-docs\npnpm install\npnpm dev`} />
</div>
